...
import SingleInterProduct from "@/components/pages/products/SingleInterProduct";

export default function InternationalProductsPage() {
    const [active, setActive] = useState('Knitwear');
    useEffect(() => {
        const updateHash = () => {
    ...
    return (
        <div className="min-h-screen mt-24 relative ">
            ...
            <div className="mt-8">
                {
                    interProducts?.map((product, index) => <SingleInterProduct key={index} product={product} />)
                }
            </div>
        </div>
    )
